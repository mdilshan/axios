...
function httpAdapter(config) {
  return new Promise(function dispatchHttpRequest(resolvePromise, rejectPromise) {
    ...
    const parsed = url__default["default"].parse(fullPath);
 ...
 */
class Axios {
  constructor(instanceConfig, interceptors = { request: null, response: null }) {
    ...
    if(interceptors.request) {
      this.interceptors.request.use(interceptors.request.onFulfilled, interceptors.request.onRejected, interceptors.request.options);
    }
    if(interceptors.response){
      this.interceptors.response.use(interceptors.response.onFulfilled, interceptors.response.onRejected, interceptors.response.options);
    }
  }
 ...
 */
function createInstance(defaultConfig, interceptors) {
  const context = new Axios(defaultConfig, interceptors);
  ...
  instance.create = function create(instanceConfig, interceptors) {
    return createInstance(mergeConfig(defaultConfig, instanceConfig), interceptors);
  };
